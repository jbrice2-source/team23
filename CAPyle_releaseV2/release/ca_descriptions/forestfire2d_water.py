...
def transition_function(grid, neighbourstates, neighbourcounts, decaygrid, wind_dir, wind_enabled):
    ...
    any_burning_neighbour = np.zeros(grid.shape, dtype=bool)
    for bd in burning_dirs:
        any_burning_neighbour |= bd
    BASE_SPREAD = 0.1
    prob_grid[any_burning_neighbour] = BASE_SPREAD

    if wind == True:
        for affected_by_wind_idx, burning_dir in enumerate(burning_dirs):
            # Find all cells that have a burning dir neighbour
            indices_of_burning_dir = np.where(burning_dir)          
            # add the neighbour contribution
            prob_grid[indices_of_burning_dir] += 0.3 * np.cos(cartesian_angles[affected_by_wind_idx]-cartesian_angles[wind_direction])      
    ...
    # UNUSED DYNAMIC WIND
    # Change wind direction with probability 0.004
...
def main():
    ...
    wind_direction = [{
        ...
        'S': 6,
        'SE': 7
    }[WIND_DIRECTION]]  # I.e if wind is north, it blows from the
